...
priority_queue<mytimer*, deque<mytimer*>, timerCmp> myTimerQueue;    // 小顶堆
...
int requestData::parse_URI()
{
    ...
    else
    {
        ...
        else
        {
            if (_pos - pos > 1)
            {
                ...
                int __pos = file_name.find('?');   // ?前是要访问的文件
                if (__pos >= 0)
                {
    ...
    else
    {
        ...
        else
        {
            ...
            if (ver == "1.0")
                HTTPversion = HTTP_10;    // 短连接
            else if (ver == "1.1")
                HTTPversion = HTTP_11;    // 长连接
            else
                return PARSE_URI_ERROR;
        }
    }
...
int requestData::parse_Headers()
{
    ...
    for (int i = 0; i < str.size() && notFinish; ++i)
    {
        switch(h_state)
        {
            ...
            case h_key:   // header中包括Accept，Cookie等关键字与对应值，对应格式，Accept: xxxxxxxx(值)
            {
                if (str[i] == ':')
                {
                    key_end = i;
    ...
    if (h_state == h_end_LF)
    {
        str = str.substr(now_read_line_begin);  // 解析报文头成功，去掉报文头。
        return PARSE_HEADER_SUCCESS;
    }
    str = str.substr(now_read_line_begin);  // 解析失败，去掉不符合格式部分
    return PARSE_HEADER_AGAIN;
}

int requestData::analysisRequest()   // 构造响应报文行，头，主体
{
    if (method == METHOD_POST)
    {
        //get content
    ...
    else if (method == METHOD_GET)
    {
        ...
        struct stat sbuf;
        if (stat(file_name.c_str(), &sbuf) < 0)   // 查看指定文件iNode的内容
        {
...
{
    //cout << "mytimer()" << endl;
    struct timeval now;   // timeval, 常用于定时，类似定时器itimerspec的timerspec，有秒，us。
    gettimeofday(&now, NULL);   // 返回标准时间到现在的总秒数（考虑时区），类似time(time_t *t)
    // 以毫秒计
    expired_time = ((now.tv_sec * 1000) + (now.tv_usec / 1000)) + timeout;  // 计算超时事件，切换到ms单位
}
...
bool timerCmp::operator()(const mytimer *a, const mytimer *b) const   // 大于逻辑实现小顶堆
{
